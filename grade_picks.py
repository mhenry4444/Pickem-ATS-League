...
def grade_picks(picks_csv_path='picks.csv', outcomes_json_path=None, matchups_file=None, override_json_path=None, week=None, output_csv_path='standings.csv'):
    # Load picks from CSV
    try:
        picks_df = pd.read_csv(picks_csv_path, index_col=False)
    except FileNotFoundError:
    ...
    if os.path.exists(output_csv_path):
        try:
            standings_df = pd.read_csv(output_csv_path, index_col=False)
        except:
...
grade_picks(week=week, outcomes_json_path=outcomes_json_path, matchups_file=matchups_file, override_json_path=override_json_path)import pandas as pd
import json
import requests
import os
